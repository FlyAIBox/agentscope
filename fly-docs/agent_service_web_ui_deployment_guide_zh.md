...
文档中的命令已于 2026-07-12 在 Linux ARM64、Python 3.11.15、uv 0.11.28、Node.js 22.14.0、pnpm 11.12.0 环境中实际执行验证。
...
脚本自动定位仓库、激活 `.venv`、检查运行依赖和端口，并依次启动 Redis、Agent Service、Web UI Backend 与 Web UI Frontend。它会复用端口上已经健康运行的服务；`stop` 只停止由脚本自身记录的应用进程，默认不会停止 Redis，以免影响持久化数据。
...
本地开发进程在对应终端按 `Ctrl+C`。Redis 容器可用以下命令停止：
...
示例默认连接 `localhost:6379`，因此 Redis 必须能从 Agent Service 所在主机通过该地址访问。
...
不要直接杀死来源不明的进程。先用 `ps -fp <PID>` 和 `/proc/<PID>/cwd` 确认进程属于哪个项目，再决定停止或改端口。
